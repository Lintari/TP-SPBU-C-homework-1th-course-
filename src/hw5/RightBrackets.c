...
int rightBrackets(char b, headStack* s) // функция которая проверяет что открывающая и замыкающая скобки одного и того же типа
{
    if (b == ')') {
        if (peek(s) == '(') {
            pop(s);
...
int main(void)
{
    ...
    while ((a = getchar()) != '\n' && a != EOF ) { // будем делать все в рамках этого цикла
        ...
        //    вообще класть открывающие мы можем сколько угодно. а вот закрывающие по логике должны быт
}
    if (getLen(s) == 0){
